...
function ThreeTabs({comp,type,offer, probs,usern, quest,debug,time,subm,titlee}) {
   ...
   useEffect(() => {
    ...
    const submitParticipation = () => {
      ...
      fetch(`http://localhost:3000/competitions/${cid}`, {
        ...
        if (type === 'Job Offer Challenge') {
          // Show success modal
          Modal.success({
            content: 'Job Application submitted',
          });
        
          // Send PATCH request to add offer to job applications
          fetch(`http://localhost:3000/users/${id}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            ...
            body: JSON.stringify({
              job_applications: [...user.job_applications,{ offer : offer}],
            }),
          })
            .then((response) => response.json())
            .then((data) => {
              history(`/dashboard/developer/${id}/profile`)
            })
            .catch((error) => {
  ...
  />
    ...
    >
      ...
      {questions.map((question)=>{
        ...
        return(
        <QuestionCompetition handleSubs={onChangeQuestionSubs}  key={question._id} questionn={question} />)
      })}
    </TabPane>
    <TabPane
      tab={
      ...
      key="3"
    >
      <DebuggerCompetition handleSub={onChangeDebuggerSubs} value={deb}/>
    </TabPane>
    <TabPane
        tab={
          <span>
      
            <Countdown  value={deadline} onFinish={success}/>
